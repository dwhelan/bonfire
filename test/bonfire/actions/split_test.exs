defmodule SplitTest do
  ...
  test "split_zero_or_more/2" do
    ...
    assert split_zero_or_more({'a', '_'}, ALPHA) == {'', 'a_'}
    assert split_zero_or_more({'ab', '_'}, ALPHA) == {'', 'ba_'}
    assert split_zero_or_more({'1', '_'}, ALPHA) == {'1', '_'}

    assert split_zero_or_more({'', '_'}, &_true/1) == {'', '_'}
    assert split_zero_or_more({'a', '_'}, &_true/1) == {'', 'a_'}
    assert split_zero_or_more({'ab', '_'}, &_true/1) == {'', 'ba_'}
  ...
  test "split_one_or_more" do
    ...
    assert split_one_or_more({'a', '_'}, ALPHA) == {'', 'a_'}
